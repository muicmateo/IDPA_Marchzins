...
const translations = {
  de: {
    ...
    kapital: "Kapital (€):",
    normalzins: "Normalzins (% p.a.):",
    bonuszins: "Bonuszins (% p.a.):",
    geburtstag: "Geburtstag (Tag):",
    geburtsmonat: "Geburtsmonat (1-12):",
    berechnen: "Berechnen",
    ergebnisPlaceholder: "Bitte geben Sie alle Werte ein und klicken Sie auf \"Berechnen\".",
    generalError: "Bitte korrigieren Sie die Fehler und versuchen Sie es erneut.",
    fehlerKapital: "Bitte geben Sie einen gültigen Kapitalbetrag ein.",
    fehlerKapitalMax: "Das Kapital ist zu hoch (max. 999.999.999 €).",
  ...
  en: {
    ...
    kapital: "Capital (€):",
    normalzins: "Normal Interest (% p.a.):",
    bonuszins: "Bonus Interest (% p.a.):",
    ...
    fehlerKapitalMax: "The capital is too high (max. 999,999,999 €).",
...
function validateGeburtstag(tag, monat) {
  ...
  if (tag < 1 || tag > tageImMonat) {
    const monatName = new Date(new Date().getFullYear(), monat - 1)
                        .toLocaleDateString(currentLang === 'de' ? 'de-DE' : 'en-US', { month: 'long' });
    return t.fehlerGeburtstagMonat.replace("{monat}", monatName).replace("{tage}", tageImMonat);
  }
  return null;
}
...
document.getElementById("berechnenButton").addEventListener("click", function() {
  ...
  const kapitalValue = parseFloat(document.getElementById("kapital").value.replace(',', '.'));
  const normalzinsValue = parseFloat(document.getElementById("normalzins").value.replace(',', '.'));
  const bonuszinsValue = parseFloat(document.getElementById("bonuszins").value.replace(',', '.'));
  const geburtstagValue = parseInt(document.getElementById("geburtstag").value);
  const geburtsmonatValue = parseInt(document.getElementById("geburtsmonat").value);

  let hasErrors = false;
  const validations = [
    { id: 'kapital', value: kapitalValue, fn: validateKapital },
    { id: 'normalzins', value: normalzinsValue, fn: (v) => validateZins(v, translations[currentLang].normalzins) },
    { id: 'bonuszins', value: bonuszinsValue, fn: (v) => validateZins(v, translations[currentLang].bonuszins) },
    { id: 'geburtsmonat', value: geburtsmonatValue, fn: validateGeburtsmonat },
    { id: 'geburtstag', value: geburtstagValue, fn: (v) => validateGeburtstag(v, geburtsmonatValue) }
  ];
  ...
  try {
    ...
    if (geburtstagValue > tageMonat) {
      const monatName = new Date(jahr, geburtsmonatValue - 1)
                          .toLocaleDateString(currentLang === 'de' ? 'de-DE' : 'en-US', { month: 'long' });
      showError('geburtstag', translations[currentLang].fehlerGeburtstagMonat
    ...
    document.getElementById("ergebnis").innerText =
      `${monatName} ${jahr} - ${currentLang === 'de' ? 'Zinsberechnung' : 'Interest Calculation'}:\n\n` +
      `${currentLang === 'de' ? 'Anzahl Tage im Monat' : 'Days in month'}: ${tageMonat}\n` +
      `${currentLang === 'de' ? 'Bonustage' : 'Bonus days'} (1. - ${geburtstagValue}.): ${tageBonus}\n` +
      `${currentLang === 'de' ? 'Normaltage' : 'Normal days'} (${geburtstagValue + 1}. - ${tageMonat}.): ${tageNormal}\n\n` +
      `${currentLang === 'de' ? 'Bonuszins-Anteil' : 'Bonus interest portion'}: ${zinsBonus.toFixed(2)} €\n` +
      `${currentLang === 'de' ? 'Normalzins-Anteil' : 'Normal interest portion'}: ${zinsNormal.toFixed(2)} €\n\n` +
      `${currentLang === 'de' ? 'Gesamtzins für' : 'Total interest for'} ${monatName}: ${gesamtzins.toFixed(2)} €`;
  } catch (error) {
    console.error('Berechnungsfehler:', error);
    showGeneralError(currentLang === 'de' ? 'Es ist ein unerwarteter Fehler aufgetreten. Bitte überprüfen Sie Ihre Eingaben.' 
